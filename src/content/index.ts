...
const projectModules = import.meta.glob<{ default: ProjectContent }>('./projects/*.json', {
	eager: true
});
const experimentModules = import.meta.glob<{ default: ExperimentContent }>('./experiments/*.json', {
	eager: true
});
...
export function loadProject(slug: string): ProjectContent | null {
	const match = Object.entries(projectModules).find(([key]) => key.includes(`${slug}.json`));
...
export function getExperiment(slug: string): ExperimentContent | null {
	const entry = Object.entries(experimentModules).find(([key]) => key.includes(`${slug}.json`));
	if (!entry) {
