...
export const load: PageServerLoad = async ({ url }) => {
	...
	const categories = Array.from(
		new Set(allPosts.map((post) => post.category ?? 'Uncategorized').filter(Boolean))
	...
	const posts = filteredPosts.slice((safePageNumber - 1) * PAGE_SIZE, safePageNumber * PAGE_SIZE);
