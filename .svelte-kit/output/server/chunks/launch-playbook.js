...
function Launch_playbook_mdx($$renderer) {
  $$renderer.push(`<p>Launching is choreography. Every step—from early prototypes to final QA—must be intentional so the audience feels the magic without sensing the chaos behind the scenes.</p> <h2>Pillars</h2> <ul><li><strong>Narrative first</strong> — Align story arcs, voice, and CTA orchestration with business goals.</li> <li><strong>Motion as language</strong> — Document entrance timings, state changes, and progressive disclosure.</li> <li><strong>Resilience</strong> — Build fallback experiences and automate regression checks for every route.</li></ul> <h3>Motion Timeline</h3> <pre class="language-ts">${html(`<code class="language-ts"><span class="token keyword">const</span> heroReveal <span class="token operator">=</span> <span class="token function">timeline</span><span class="token punctuation">(</span><span class="token punctuation">[</span>
	...
	<span class="token punctuation">]</span>
<span class="token punctuation">]</span><span class="token punctuation">)</span><span class="token punctuation">;</span></code>`)}</pre> <h2>QA Rituals</h2> <ol><li>Automated visual regression with multiple theme snapshots.</li> <li>Performance budgets enforced via Lighthouse CI.</li> <li>Manual run-through with screen readers and reduced-motion settings.</li></ol> <p>The deliverable? A reusable launch playbook that meets marketing, product, and engineering expectations while delighting users.</p>`);
}
export {
  Launch_playbook_mdx as default,
  metadata
};
