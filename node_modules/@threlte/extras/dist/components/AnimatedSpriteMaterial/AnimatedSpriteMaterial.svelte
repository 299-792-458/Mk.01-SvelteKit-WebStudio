...
const { start, stop } = useFrame(() => {
    ...
    const start = direction === 'forward' ? frameTag?.from ?? startFrame : frameTag?.to ?? numFrames - 1;
    const end = direction === 'forward' ? frameTag?.to ?? numFrames - 1 : frameTag?.from ?? startFrame;
    setFrame(frame);
    switch (direction) {
        case 'forward':
            currentFrame += 1;
